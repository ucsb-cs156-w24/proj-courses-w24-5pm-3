...
describe("UpdateCoursesForRangeOfQuartersJobForm tests", () => {
  ...
  test("renders without crashing when fallback values are used", async () => {
    ...
    expect(
      await screen.findByTestId(/BasicSearch.StartQuarter-option-1/),
    ).toHaveValue("20212");
    expect(
      await screen.findByTestId(/BasicSearch.EndQuarter-option-0/),
    ).toHaveValue("20211");
    expect(
      await screen.findByTestId(/BasicSearch.EndQuarter-option-1/),
    ).toHaveValue("20212");
  });
});
